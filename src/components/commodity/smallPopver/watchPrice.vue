   ...
   <!-- <div class="popover-main">
    ...
    <div class="popover-main">
      <el-table
        v-loading="loading"
        :data="priceList"
        style="border:1px solid #dcdfe6;border-bottom: none;width: 630px;"
        :max-height="300"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
              <span style="margin-left: 10px" v-if="item.which=='momey'">
               <el-input placeholder="请输入佣金" class="GetPrice" v-model="priceListUpdata[scope.$index]" size="mini" style="width: 90%;" v-if="PproductgetByIdResult.item.length>0"></el-input>
            ...
            <span style="margin-left: 10px" v-else-if="item.which=='salePriceView'">
               <el-input placeholder="请输入特卖价" v-model="activeUpdata[scope.$index]" value="" size="mini" style="width: 90%;" v-if="PproductgetByIdResult.item.length>0"></el-input>
  ...
  data () {
    return {
     ...
     // priceList:[{muchG:"规格一",sale:'109.00',ownPrice:'100'},{muchG:"规格二",sale:'199.00',ownPrice:'200'},{muchG:"规格三",sale:'1999.00',ownPrice:'300'},],
      classArr:[{title:'规格',which:'normalStr',width:'200'},{title:'市场价',which:'marketPriceView',width:'100'},{title:'成本',which:'costPriceView',width:'100'},{title:'特卖价',which:'salePriceView',width:'100'},{title:'佣金',which:'momey',width:'100'}]
    }
  },
  computed:{
    ...mapGetters([
...
<style scoped lang="scss">
  .demo{
   // width: 400px;
    ...
    .el-button{
      margin:0 auto;display: block;margin-bottom: 15px;
    }
  }
  .popover-head{
    width: 100%;
