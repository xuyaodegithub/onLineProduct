    ...
    <div class="seach-list">
      ...
      <el-table
        ...
        <el-table-column
          ...
          :key="index"
          :width="item.width"
          :show-overflow-tooltip ="item.which!='sort' && item.which!='isSetTop' && item.which!='image'">
          <template slot-scope="scope">
            ...
            <span style="margin-left: 10px" v-else-if="item.which==='sort'">
                  <el-input placeholder="请输入..." :value="scope.row[item.which]" size="mini" style="width: 90%;" @blur="changeSort($event,scope.row)" @change="changeValue"></el-input>
                </span>
          ...
          <template slot-scope="scope">
            <el-button
             ...
             <el-button
              ...
              type="warning"
              icon="el-icon-sort"
              @click="ToupDataStatus([scope.row])" plain>更改状态</el-button>
             <el-button
              size="mini"
              type="primary"
              icon="el-icon-edit"
...
//      },
      ...
      watchShare(row){
        this.$store.commit('activeIdchange','https://ol-site.olquan.cn/weixin/auth?view=https://ol-h5-preview.olquan.cn/demo/iscroll/id/'+row.productId+'?isShare=1&type=4')
