<template>
    <div class="flex" id="newnews">
      <div class="left">
        <div v-for="(item,index) in arrList" :key="index" class="cu" :class="{'active' : !item.image,'addborder' : oneNum===index}" @click="choseThis(index)">
          <img :src="isImage" alt="" v-if="isImage">
          <div class="zhezhao" v-if="isImage"></div>
          <div class="zhezhao1" v-if="index!==0 && !isImage"></div>
      ...
      <div class="right">
        ...
        <el-input v-model="isouther" placeholder="请输入作者(最多8字符)" class="smallTitle" maxlength="64" @input="changeTitle(2)"></el-input>
        ...
        <div class="labeil">
          <label>封面:</label> <el-button plain size="mini">从图片库选择</el-button>
          <img :src="isImage" alt="" v-if="isImage" style="display: block;width: 180px;height:90px;margin:10px 0 0 120px;">
        </div>
        <div class="labeil">
          <label>是否显示封面:</label>
        ...
        <div class="btn">
          <el-button type="success" style="margin-left: 100px;" @click="addNewnews()">保存</el-button>
          <el-button plain >预览</el-button>
          <el-button plain>保存并群发</el-button>
        </div>
      </div>
...
//        {
//          title:'',
//          image:'https://ol-quan2017.oss-cn-shanghai.aliyuncs.com/e86e61460d19e9ee4a72a62e9cf05106f9319a5e',
...
//            },
            loading: true,
            name: 'file',
            action:'/apis/admin/weixin/messageSend/uploadImg',
            response: (res) => {
              console.log(res)
              return res.result
            }
...
//    }
    ...
    if(this.type==='add'){
      ...
      this.arrList=[{
        title:'',
        thumb_media_id:'md1Kxh0aHlPrAA2eXkKzoSRLUMiASlKASJZZbTNnmxo',//封面素材id
        author:'',//作者//否传
        digest:'',//图文消息的摘要，仅有单图文消息才有摘要，多图文此处为空。如果本字段为没有填写，则默认抓取正文前64个字。//否传
        show_cover_pic:1,//是否显示封面，0为false，即不显示，1为true，即显示
        content:'',//图文消息的具体内容，支持HTML标签，必须少于2万字符，小于1M，且此处会去除JS,涉及图片url必须来源 "上传图文消息内的图片获取URL"接口获取。外部图片url将被过滤。
        content_source_url:'',//图文消息的原文地址，即点击“阅读原文”后的URL
        need_open_comment:0,//Uint32 是否打开评论，0不打开，1打开//否传
        onlyans_can_comment:'',//Uint32 是否粉丝才可评论，0所有人可评论，1粉丝才可评论//否传
      }]
    }else{
      this.oneNum=0
      ...
      this.radio2=this.arrList[0].onlyans_can_comment
//      this.isImage=this.arrList[0].title
...
'popoverAlert'
    ...
    changeTitle(e){//改变标题
      ...
      else if(e===6) this.arrList[this.oneNum].onlyans_can_comment=this.radio2
    ...
    addNewnews(){
//      this.arrList.content.toString().replace(/(src)/g,'data-src')
      let data={
        "paramStr":JSON.stringify({"articles":this.arrList})
      }
