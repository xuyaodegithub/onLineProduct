<template>
  <div id="zujianData">
    <div v-if="commodityResult.modelSampleCode !=='catlist1'" class="banner">
      ...
      <div class="img">
        ...
        <div style="display: flex;position: relative">
          ...
          <ul id="imgList">
            <li v-for="(item,key) in commodityResult.contents" @click="address(key)"
                v-if="commodityResult.contents.length>0" :class="{activete : num===key}" class="cu">
              <img
                :src="item.image ?item.image : 'http://ol-quan2017.oss-cn-shanghai.aliyuncs.com/23c31d63de867f568af9d1b8a3d193573b456a41'"
                alt=""/>
            ...
            <li>
              <el-button style="margin-top:25px;" size="mini" @click="addimg()">上传图片</el-button>
              <!--<el-button type="success" plain style="margin-top: 5px;margin-left: 0;" size="mini" @click="addGoodsList()">关联商品</el-button>-->
            </li>
          </ul>
          <el-upload
        ...
        <div class="anchor-b">
          ...
          <div v-if="radio2===29">
            <label style="margin-right: 20px;width: 60px;text-align: right;display: inline-block;margin-bottom: 10px;">活动选择:</label>
            <el-select v-model="value" placeholder="请选择活动" size="mini" @change="addImgUrl(value,1)">
              <el-option
          ...
          <div v-if="radio2===34">
            <label style="margin-right: 20px;width: 60px;text-align: right;display: inline-block;margin-bottom: 10px;">活动选择:</label>
            <el-select v-model="value1" placeholder="请选择活动" size="mini" @change="addImgUrl(value1,2)">
              <el-option
                v-for="item in CouponActiveListResult.rows"
                :key="item.id? item.id : ''"
                :label="item.name ? item.name : ''"
                :value="item.id ? item.id : ''">
              </el-option>
          ...
          <p>
            ...
            <el-input v-model="ImgUrlResult" placeholder="请输入内容" size="mini" :disabled="openOrclose" @blur="change()"
                      v-if="radio2!==17"></el-input>
            <el-input v-model="input" placeholder="请输入内容" size="mini" :disabled="openOrclose" @blur="change()"
                      v-if="radio2===17"></el-input>
          ...
          <p><label style="margin-right: 23px;display: inline-block;width: 60px;text-align: right;margin-top: 10px">图片边距:</label>
            <el-input v-model="lineData" placeholder="请输入内容" size="mini" @blur="change2()"></el-input>
          </p>
        </div>
      </div>
    ...
    <div id="goodChangeList" v-else>
      ...
      <div class="flmcList">
        <p><label>分类名称:</label>
          <el-input v-model="input1" placeholder="请输入分类名称" size="mini"></el-input>
          <el-button v-if="createOrUpdate" type="primary" plain size="mini" style="margin-left: 20px"
                     @click="createTag()">创建分类
          </el-button>
          <el-button v-if="!createOrUpdate" type="primary" plain size="mini" style="margin-left: 20px"
                     @click="changeTag()">确认修改
          </el-button>
        ...
        <el-button
          ...
          {{tag.title}}<i class="el-icon-close" style="float: right;font-size: 14px;"
                          @dblclick.pervent="CloseTag(key)"></i>
        </el-button>
      </div>
      <div class="addGoodList">
        <p style="font:14px/28px '微软雅黑'"><label>产品列表:</label>
          <el-button type="primary" plain size="mini" icon="el-icon-plus" style="float: right;" @click="addPucter()">
            添加关联产品
          </el-button>
        </p>
        <el-table
          ...
          <el-table-column label="操作" width="100">
            <template slot-scope="scope">
              <el-button
                size="mini"
                plain
                @click="handleEdit(scope.$index, scope.row)" style="width: auto;">移除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
//          {title:'粉领专享', which:5},
    ...
    computed: {
      ...mapGetters([
        ...
        'ImgUrlResult', 'CouponActiveListResult'
      ]),
    },
    methods: {
      ...mapActions([
        'commodityActions', 'popoverAlert', 'listActiveActions', 'CouponActiveListActions'
      ]),
      ...
      address(key) {
        ...
        if (this.commodityResult.contents[key].type === 1) {
          ...
          this.radio2 = 33
        } else if (this.commodityResult.contents[key].type === 22) {
          this.radio2 = 34
...
//          this.input='http://ol-h5.olquan.cn/supervisor/buyPink'
          ...
          this.input = 'https://ol-h5-preview.olquan.cn/try/newCenter'
          this.addType(1)
        } else if (key === 20) {
          ...
          this.input = 'https://ol-h5-preview.olquan.cn/index/pinkIndex'
          this.addType(20)
        } else if (key === 31) {
          this.input = '#'
          this.addType(0)
